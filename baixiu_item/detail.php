<?php
  ...
  $sql = "SELECT c.`name`, c.`id`, p.title, u.`nickname`, p.created, p.views, p.content,(SELECT COUNT(id) FROM comments WHERE post_id= p.id) AS commentsCount FROM posts p
...
<?php 
  ...
  // $postId = $_GET['postId'];
  // include './static/select.php';
  // $sql = "SELECT c.`name`, c.`id`, p.title, u.`nickname`, p.created, p.views, p.content, (SELECT COUNT(id) FROM comments WHERE postId = p.id) AS commentsCount FROM posts p
  // LEFT JOIN categories c ON c.`id` = P.`category_id`
  // LEFT JOIN users u ON u.`id` = p.`user_id` WHERE p.`id` = {$postId}";
  // $getArr = select($sql)[0];
?>
    ...
    <div class="content">
      <div class="article">
        <div class="breadcrumb">
          <dl>
            <dt>当前位置：</dt>
            <dd><a href="list.php?catagoryId=<?php echo $getArr['id']; ?>"><?php echo $getArr['name']; ?></a></dd>
            <dd><?php echo $getArr['title']; ?></dd>
          </dl>
        </div>
        <h2 class="title">
          <a href="javascript:;"><?php echo $getArr['title']; ?></a>
        </h2>
        <div class="meta">
          <span><?php echo $getArr['nickname']; ?> 发布于 <?php echo $getArr['created']; ?></span>
          <span>分类 : <a href="list.php?catagoryId=<?php echo $getArr['id']; ?>"><?php echo $getArr['name']; ?></a></span>
          <span>阅读: (<?php echo $getArr['views']; ?>)</span>
          <span>评论: (<?php echo $getArr['commentsCount']; ?>)</span>
          <p class="brief"><?php echo $getArr['content']; ?></p>
        </div>
      </div>

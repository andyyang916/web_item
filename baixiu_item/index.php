<?php
  ...
  $sql = "SELECT p.title, p.feature, p.created, p.content, p.views, p.likes, c.`name`, u.`nickname`, (SELECT COUNT(id) FROM comments WHERE post_id= p.id) AS commentsCount FROM posts p LEFT JOIN 
  categories c ON c.`id` = p.`category_id` LEFT JOIN users u ON u.id = p.`user_id` WHERE p.`category_id` != 1 ORDER BY p.`created` DESC LIMIT 5";
  $postArr = select($sql);
  // print_r($postArr);
